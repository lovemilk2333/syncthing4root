...
若你之前使用 `com.nutomic.syncthingandroid`, 可在重启前可将现有配置迁移至本模块 (其他应用可以使用类似操作, 请确保目标文件夹内目录结构类似于 `cert.pem  config.xml  https-cert.pem  https-key.pem  index-v0.14.0.db  key.pem`):

```sh
# 1. 复制配置文件至 syncthing home 目录
cp -r /data/data/com.nutomic.syncthingandroid/files/* /data/adb/io.github.lovemilk2333.root_module.syncthing4root/syncthing/home/

# 2. 修改目录及文件权限
chmod 0755 /data/adb/io.github.lovemilk2333.root_module.syncthing4root/syncthing/home
chmod 0644 /data/adb/io.github.lovemilk2333.root_module.syncthing4root/syncthing/home/*
```
...
迁移后重启设备或手动启动 Syncthing 即可

## 手动管理

- **打开 Web 界面**: 在模块管理器中点击 action 按钮, 自动跳转至 Syncthing Web UI
- **数据目录**: `/data/adb/io.github.lovemilk2333.root_module.syncthing4root/syncthing/home/` (内含 `config.xml` 等)
